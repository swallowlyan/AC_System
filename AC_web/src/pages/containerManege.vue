        ...
        <el-form :inline="true" :model="searchItem" ref="searchItem">
                  ...
                  <el-form-item label="适用终端">
                    <el-select v-model="searchItem.terminalId" :placeholder="terminalArr.title">
                      <el-option v-for="item in terminalArr.options"
                                 :key="item.value"
                                 :label="item.label"
                                 :value="item.value">
                      </el-option>
                  ...
                  <el-form-item label="容器状态">
                    <el-select v-model="searchItem.status" :placeholder="statusArr.status">
                      <el-option v-for="item in statusArr.options"
                                 :key="item.value"
                                 :label="item.label"
                                 :value="item.value">
                      </el-option>
...
<el-button-group>
          <el-button type="success" round size="small" icon="el-icon-refresh" @click="search(1,10)">刷新</el-button>
  <el-button type="primary" round size="small" icon="el-icon-plus" @click="add('dialogForm')">新增</el-button>
  ...
  <el-form :model="dialogForm" :rules="dialogRules" ref="dialogForm" label-width="100px">
    ...
    <el-form-item label="适用终端" prop="terminal">
      <el-col :span="15">
<el-select v-model="dialogForm.terminal" :placeholder="terminalArr.title" style="width:100%">
        <el-option v-for="item in terminalArr.options"
                                 :key="item.value"
                                 :label="item.label"
                                 :value="item.value">
    ...
    export default {
      ...
      data(){
          return {
            ...
            tableData:[
              {name:'MathCAD',version:'1.0',createTime:'2019-08-01',factory:'厂商1',terminal:'终端1',status:'在线',instructions:"测试用例"},
              {name:'MathCAD',version:'1.0',createTime:'2019-08-01',factory:'厂商1',terminal:'终端1',status:'在线',instructions:"测试用例"},
              {name:'MathCAD',version:'1.0',createTime:'2019-08-01',factory:'厂商1',terminal:'终端1',status:'在线',instructions:"测试用例"},
              {name:'MathCAD',version:'1.0',createTime:'2019-08-01',factory:'厂商1',terminal:'终端1',status:'在线',instructions:"测试用例"},
              {name:'MathCAD',version:'1.0',createTime:'2019-08-01',factory:'厂商1',terminal:'终端1',status:'在线',instructions:"测试用例"},
